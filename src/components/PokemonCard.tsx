...
export const PokemonCard = ({ pokemon, isSelected, onClick }: PokemonCardProps) => {
  ...
  return (
    <button
      ...
      className={`group relative flex h-[84px] flex-col items-center justify-end rounded-lg border bg-ink-850 pt-1.5 transition-all duration-150 ${
        isSelected
    ...
    >
      <img
        ...
        alt={pokemon.name}
        loading="lazy"
        className="h-11 w-11 object-contain drop-shadow-[0_4px_8px_rgba(0,0,0,0.45)]"
        style={triedFallback ? undefined : { imageRendering: "pixelated" }}
      />
      <span
        className={`mt-1 w-full truncate rounded-b-lg px-0.5 py-0.5 text-center text-[9px] font-semibold uppercase leading-tight tracking-tight ${
